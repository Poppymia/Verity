...
import { parseClaudeJsonObject } from './claudeJsonParse.js';
...
export function createApp({ fetchImpl = fetchDefault, config = {} } = {}) {
  const app = express();
  ...
  app.post('/api/verify-claim', async (req, res) => {
    ...
    try {
      ...
      if (GOOGLE_FACT_CHECK_API_KEY) {
        ...
        const fcResp = await fetchImpl(url);
        if (fcResp.ok) {
      ...
      const claudeResp = await fetchImpl(CLAUDE_MESSAGES_ENDPOINT, {
        method: 'POST',
        headers: {
      ...
      const rating =
        parsed.rating === 'verified' ||
        parsed.rating === 'false' ||
        parsed.rating === 'questionable'
          ? parsed.rating
          : 'questionable';
